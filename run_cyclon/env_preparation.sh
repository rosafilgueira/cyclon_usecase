...
mkdir -p $STAGED_DATA/cyclon
mkdir -p $STAGED_DATA/cyclon/results

cp -r $INPUT_DIR/workflow_cmip6/* $STAGED_DATA/cyclon/.

TRACKDIR=$STAGED_DATA/cyclon/
WORKDIR=$STAGED_DATA/cyclon/results
CONFIGFILE=${STAGED_DATA}/cyclon/cyclone_config_CMIP6.json
RUNCONFIG=${STAGED_DATA}/cyclon/config_cmip6.txt
INPUTFILELIST=${STAGED_DATA}/cyclon/input_files.txt
...
export CONCATFILES=${TRACKDIR}/ConcatenationFichiers.py
export CREATEMAPS=${TRACKDIR}/main_cartes.py
...
echo $model > $STAGED_DATA/cyclon/model.txt
