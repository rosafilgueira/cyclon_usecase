...
for f in allfiles:
   ...
   if var == orog or var == lsm :
     ...
     try: 
       dset = xr.open_dataset(f, chunks={'time': '100MB'}, mask_and_scale=False, decode_times=True, decode_coords=True)
     except:
       dset = xr.open_dataset(f, mask_and_scale=False, decode_times=True, decode_coords=True)
   try:
   ...
   if minlon > maxlon or minlon < 0:
     if var == ua500 or var == va500 or var == zg1000 :
       level=500.0
       dset = dset.sel(time=slice(starttime,endtime), plev=slice(level*100+1,level*100-1), lat=slice(minlat,maxlat)).squeeze(dim=None, drop=True)
     elif var == psl :
     ...
     if var == ua500 or var == va500 or var == zg1000 :
       level=500.0
       dset = dset.sel(time=slice(starttime,endtime), plev=slice(level*100+1,level*100-1), lon=slice(minlon,maxlon), lat=slice(minlat,maxlat)).squeeze(dim=None, drop=True)
     elif var == psl :
...
# Reorder longitudes if needed, and subset longitudes in that specific case differently (need to do it on local file for reasonable performance)
   if minlon > maxlon or minlon < 0:
     ...
     if var == orog or var == lsm :
       ...
       try:
         dsetl = xr.open_dataset("results/"+fb, chunks={'time': '100MB'}, mask_and_scale=False, decode_times=True, decode_coords=True)
       except:
         dsetl = xr.open_dataset("results/"+fb, mask_and_scale=False, decode_times=True, decode_coords=True)
...
try:
  dsmerged = xr.open_mfdataset(files, chunks={'time': '100MB'}, mask_and_scale=False, decode_times=True, decode_coords=True, combine='by_coords')
except:
