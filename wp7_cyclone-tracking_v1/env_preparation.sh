...
mkdir -p $STAGED_DATA/cyclone_workflow
mkdir -p $STAGED_DATA/cyclone_workflow/output

echo $INPUT_DIR
echo $STAGED_DATA

cp -r $INPUT_DIR/* $STAGED_DATA/cyclone_workflow/.

TRACKDIR=$STAGED_DATA/cyclone_workflow/
WORKDIR=$STAGED_DATA/cyclone_workflow/output
CONFIGFILE=${STAGED_DATA}/cyclone_workflow/cyclone_config_CMIP6.json
RUNCONFIG=${STAGED_DATA}/cyclone_workflow/config_cmip6.txt
INPUTFILELIST=${STAGED_DATA}/cyclone_workflow/input_files.txt
...
echo $model > $STAGED_DATA/cyclone_workflow/model.txt
